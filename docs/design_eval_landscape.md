...
> **一页式索引见 [`design_eval_summary.md`](design_eval_summary.md)** —— 每个 benchmark 一行，四张表（身份与协议 / 评估与判据 / 可得性与成本 / credibility 与 maturity）。本文件仍是 source of truth，索引里每行都指回这里的小节号。

**日期**：2026-09-10
**状态**：广度调研完成，**五类全部做过源码级核实**（2026-09-12/13）——binder / ProtDBench / A-CODE（§9）、单体 / 无条件 / motif scaffolding（§13）、peptide / miniprotein（§14）、enzyme / ligand-conditioned（§15）、antibody / nanobody（§16）。事实逐条带证据标记，见文首。
...
| **antibody** | 7 | §10.1–10.6、§10.12；**任务拆分与源码核实见 §16** |
| **nanobody** | 8 | §10.15 汇总，详见 §10.6–10.11、§10.13、§9.2；**单列一节见 §16.6** |
...
**peptide 与 miniprotein 见 §14**（已按任务拆成五类并核实）。要点：**"miniprotein binder"与本表的 binder 线是同一件事，不应重复建设**；肽类里唯一值得留意的是 PepGLAD 的重建划分。
...
**这份文档是 landscape survey —— 系统整理别人已经怎么做 benchmark。** 现阶段的目标是尽可能完整地记录领域现状，而不是提前敲定一套"最严谨"的基准。

Proteo-AA 的相关性、当前兼容性、可能的优劣势都作为**标注**记录，很有价值，但**不是判断某个 benchmark 是否值得记录或是否优先的标准**。一个基准只测骨架、只测序列、不覆盖我们的 full-atom claim、或与当前接口不完全兼容，**都不构成降级理由**——那些是它的性质，照实记下来即可。

每条尽量回答六件事：它实际测什么能力、领域里怎么用、与其他 benchmark 是否重复、我们能否公平跑、跑了只覆盖我们哪一部分能力、有无理由预期我们表现好或差。

**现阶段不拼装成一套面向 Proteo-AA 的 evaluation suite**，选型留到各类都调研完统一 review。
...
> **已由 §14 取代。** 该节做了源码级核实并把任务拆成五类（linear binder / 环肽 / 重建 / 无条件 / miniprotein），下表仅作索引。
> 一处要补：PepGLAD **有两套测试集**——论文主报的 LNR 93 条（Zenodo）和随仓库发布的 PepBDB 划分 190 条，见 §14.3。
...
> **已由 §16 扩充。** 该节把这一类拆成六种任务（结构预测对接 / CDR loop 设计 / 序列设计逆折叠 / 共设计与 binder / 亲和力成熟 / 纳米抗体），并划清了三条界线（没有基准做完整抗体从头设计；抗体与纳米抗体不共用协议；预测对接与生成式设计分开），另标出了固定 test set / 排行榜 / 社区挑战的分布。本节保留原有逐条明细，两节内容一致。
...
> **已由 §15 扩充。** 该节把这一类拆成五种任务（催化 motif 支架 / 配体条件生成 / 底物条件设计 / 口袋重设计 / 序列功能生成），并新增了「是否真被后续工作复用」这一维度。本节保留原有逐条明细，两节内容一致。
...
| **RFdiffusion 25 题 motif 集 / Genie2 的 24 题变体** | **前身集合**。MotifBench 明说建立它是因为"各家评估方式差异大导致没法比"；**就我们的 benchmark 选型而言，MotifBench 是更标准化、优先级更高的 successor**（不代表领域已完全弃用这两个集合） | 在 RFdiffusion SI 里 | 有 | `EXPLICIT_SOURCE` |
...
**MotifBench（30 题）**
- **在目前核实过的候选里**，唯一同时满足"固定案例集 + 独立维护 + 活跃排行榜 + 明确 score 公式"四条的，报出来审稿人认
...
**La-Proteina 的 all-atom co-designability**
> **类别注意：这是一个 evaluation protocol / metric，不是与 MotifBench 同层级的固定 benchmark。** 它没有题面——无条件生成本来就没有测试集（见 §13.5）。列为 `STRONG_CANDIDATE` 的理由是**用途**：它是目前唯一能把我们的 full-atom claim 变成数字的指标，而不是因为它能提供可横比的题目。

- **在已核实的候选里唯一直接考"模型自己出的序列 + 全原子 RMSD"的指标**，正对我们的 claim
- 定义清楚（ESMFold 折模型自己的序列，全原子 RMSD < 2 Å，不设 pLDDT 门槛）`EXPLICIT_SOURCE`
- 不需要 motif 适配，**接入成本在这一类里最低**
- 局限：论文自带指标，无社区题面，横向比较对象有限；报的时候要说明是按该协议自测，不是在公共基准上排名
...
**RFdiffusion 25 题 / Genie2 24 题** —— 就选型而言 MotifBench 是更标准化的 successor，优先级更高；这两个集合在文献里仍有人用，只是我们没有理由优先跑。
...
理由是这两个**恰好互补且不重叠**：
- MotifBench 是**固定 benchmark**，给**外部可信度**（固定题面、排行榜、审稿人认），代价是测不到侧链
- all-atom co-designability 是**评估协议**，给**claim 对位**（考的就是共设计序列 + 全原子），代价是没有题面、社区认可度弱

**两者不同层级**：一个提供"考哪些题"，一个提供"怎么判分"。并列使用不冲突，但不要在同一张表里当同类比较。
...
# 十四、配方明细：peptide / miniprotein

第三轮核实（2026-09-13）。只做调研与协议核准，**不建实现**。

## 14.0 先把任务拆开——避免和 binder 线重复计数

这一类里"peptide"和"miniprotein"常被混在一起说，但它们是**五个不同的任务**，能力要求和评估方式都不一样。

| 类别 | 典型长度 | 条件是什么 | 代表 |
|---|---|---|---|
| **A. linear peptide binder** | ≤ 25–30 aa | 靶点蛋白 | BOND-PEP |
| **B. cyclic / macrocyclic peptide design** | 12–18 aa | 靶点蛋白 + **环化拓扑** | RFpeptides、Latent-X 大环肽档 |
| **C. peptide–protein 重建 / 条件生成** | 4–30 aa | **天然复合物里的受体**（有真值肽可比） | PepBench/PepGLAD、DiffPepBuilder |
| **D. unconditional peptide generation** | — | 无 | **没查到公认基准**（见 §14.5） |
| **E. miniprotein binder** | 40–140 aa | 靶点蛋白 | AlphaProteo / ProtDBench / BindCraft / Cao |

### 关于 E：inventory 上与 §9 合并处理

**在目前收集到的 benchmark 里，E 类的任务设定与 §9 的 binder 线基本重合**，所以本清单把它们合并记录，不另立条目。`CODE_VERIFIED`（就已核实的这些集合而言）

依据是这些集合的设计长度：AlphaProteo 40–140、Cao 2022 的 50–65、BoltzGen 80–140——落在同一区间，条件化方式（给靶点结构 + hotspot）也相同。

> 这是**对当前 inventory 的处理方式，不是在给领域术语下定义**。"miniprotein"在文献里有自己的用法（强调尺寸、稳定性、可化学合成等），只是在**这批 benchmark 的任务设定层面**看不出与 protein binder 的区分。

**就目前核实的这些 benchmark 而言**，长度分布是分开的：肽类集中在 ≤ 约 30 残基，miniprotein binder 在 ≥ 40 残基，没有横跨两者的集合。`CODE_VERIFIED`（限于已核实范围）

## 14.1 Proteo-AA 的表示能力边界（问题六的回答）

这一节决定了 A–D 各类我们能不能**公平**参赛。全部 `CODE_VERIFIED`。

| 能力 | 现状 | 依据 |
|---|---|---|
| **标准 20AA** | ✅ 支持 | `sidechain/instantiate.py` 的 `STD_AA_3` 恰好 20 项；AA head `no_bins: 20`（`heads.py:129`） |
| **非天然氨基酸** | ❌ **不支持** | 同上，字母表里没有，AA head 也输出不了 |
| **链长** | ✅ 无结构性限制 | 无硬编码上限；短肽只是 token 数少 |
| **多链条件化** | ✅ 支持 | binder 设计就是"设计链 + 靶点链作为上下文"，§9 已在跑 |
| **头尾环化 / 大环拓扑** | ❌ **不支持** | 相对位置编码是标准 AF3 式**线性**编码（`Protenix/.../embedders.py:134`，`r_max=32`，对残基序号差做 clamp），**没有环状 offset 模式** |
| **显式共价键（非骨架）** | ⚠️ **机制在上游，我们没接** | Protenix 的 trunk 有 `token_bonds` 输入并加进 pair 表示（`protenix.py:216,225`），但 `pxdesign_train` 里**一处引用都没有**；上游 `data/utils.py:488` 说明它抓的是**配体间的键** |
| **二硫键约束** | `UNRESOLVED` | 未核实我们的数据路径是否会把二硫键写进 `token_bonds` |
| **残基级固定原子（motif 式）** | ⚠️ 机制存在但不在本线 | 见 §13.4：`stage4.py` 有，`cogenerate.py` 没有 |

### 为什么环化是 `FUTURE` 而不是 `NEEDS_ADAPTER`

看 RFdiffusion 怎么做的：`cyclic_reses` 这个参数**一路穿进模型内部**——`RoseTTAFoldModel.forward(..., cyclic_reses)` → `latent_emb(msa, seq, idx, cyclic_reses)` → `Track_module`，`Embeddings.py` 里有专门的 "cyclic peptide" 分支。`CODE_VERIFIED`

同理 RFpeptides 用的验证器是 **AfCycDesign**（带环状位置编码的 AF2 变体），论文明说原版 AF2 表示不了头尾相接的大环。`EXPLICIT_SOURCE`

**所以环化不是输入格式问题，是位置编码层的模型改动。** 写个 adapter 解决不了——生成出来的"环肽"在我们的表示里就是一条普通线性链，首尾不闭合，验证器一折就散。→ **类别 B 对我们是 `FUTURE`**。

> 相对地，**ProtDBench 的评估侧支持环肽**：`main_af2_monomer.py:146` 有 `if is_cyclic: add_cyclic_offset(prediction_model)`，默认 `False`。`CODE_VERIFIED`
> 也就是说**打分那一端不是瓶颈，生成那一端才是**。

## 14.2 类别 A：linear peptide binder —— BOND-PEP

| 字段 | 内容 | 标记 |
|---|---|---|
| **task** | **序列优先**的肽 binder 设计：给靶点，直接生成肽序列，**不生成结构** | `EXPLICIT_SOURCE` |
| **test cases** | **193 对**蛋白–肽，从 RCSB 挖，MMseqs2 按 **30% 最低同一性**对靶点序列聚类**后**再划分（同源物留同簇）。严格档筛选：BSA ≥ 400 Å²、肽 ≤ 25 aa、靶点 ≥ 30 aa | `EXPLICIT_SOURCE` |
| **case selection** | 天然复合物挖出来的（见 §14.4 的性质判断） | `EXPLICIT_SOURCE` |
| **input conditioning** | 靶点序列 | `EXPLICIT_SOURCE` |
| **expected output** | **肽序列**（无结构） | `EXPLICIT_SOURCE` |
| **sample count** | 每靶点取前 8 条候选 | `EXPLICIT_SOURCE` |
| **sequence design route** | 模型直接出序列，无逆折叠环节 | `EXPLICIT_SOURCE` |
| **evaluator** | **AlphaFold-Multimer 经 ColabFold**，默认设置；取最终 5 个输出里**最高 ipTM** | `EXPLICIT_SOURCE` |
| **threshold** | **相对阈值**：`reference-beating ipTM success@8` —— 前 8 条里至少一条的 ipTM **高于同一协议下该靶点 PDB 参考肽自己的 ipTM**。**成功线逐靶点浮动，没有固定数值** | `EXPLICIT_SOURCE` |
| **aggregation** | 靶点级成功率。结果：BOND-PEP 65.80%、RFdiffusion 37.31%、PepMLM 35.75%、PepPrCLIP 19.17% | `EXPLICIT_SOURCE` |
| **wet-lab status** | 另有一小组实验支持的外部面板，仅事后评估 | `EXPLICIT_SOURCE` |
| **code / data** | **Zenodo `10.5281/zenodo.19841318`，无 GitHub** | `EXPLICIT_SOURCE`（未下载核对，`UNRESOLVED`） |
| **Proteo-AA** | `OUT_OF_SCOPE` 作为生成任务——**它是序列生成基准，不产结构**，我们是结构-序列共设计，跑它等于只用我们的 AA head、丢掉结构分支。作为**评估协议**（相对 ipTM 阈值）可参考 | — |

## 14.3 类别 C：peptide–protein 重建 —— PepBench/PepGLAD 与 DiffPepBuilder

### PepBench / PepGLAD

| 字段 | 内容 | 标记 |
|---|---|---|
| **task** | 给天然复合物的受体，**重建**那条肽（序列 + 全原子结构） | `EXPLICIT_SOURCE` |
| **test cases** | **两套，别混**：<br>① **LNR 93 条**（Tsaban 2021 专家整理集）——论文主报的测试集，**不在仓库里，在 Zenodo**<br>② **PepBDB 划分，测试 190 条**——`datasets/pepbdb/test.txt` **随仓库发布**，格式 `<id>\t<受体链>\t<肽链>\t<含非标准AA标志>` | ① `EXPLICIT_SOURCE`（Zenodo README）<br>② `CODE_VERIFIED`（190 行） |
| **case selection** | 2023-12-08 前的 PDB 二聚体，去 90% 序列冗余，肽 4–25 残基，受体 > 30 残基 → 6,105 个非冗余复合物；按受体 **> 40% 同一性**聚类划分（训练 4,157 / 验证 114） | `EXPLICIT_SOURCE` |
| **input conditioning** | 受体结构（天然复合物里的） | `EXPLICIT_SOURCE` |
| **sample count** | 每个测试案例采 **40 条** | `EXPLICIT_SOURCE` |
| **evaluator** | **没有结构预测验证器**——直接对着晶体结构里的肽打分 | `CODE_VERIFIED`（`cal_metrics.py`） |
| **metric / threshold** | Cα RMSD 分档成功标志 **≤2.0 / ≤5.0 / ≤10.0 Å**；全原子 RMSD；AAR 与滑动 AAR；多样性与一致性；**PyRosetta ΔG，成功 = ΔG < 0**；DockQ（>0.23 可接受 / >0.49 中等） | `CODE_VERIFIED` |
| **非标准 AA** | **索引文件里第四列就是"肽是否含非标准氨基酸"的标志位** —— 说明数据集里确实有这类案例 | `CODE_VERIFIED` |
| **code / data** | `github.com/THUNLP-MT/PepGLAD`，PepBDB 划分随仓库；LNR 与 ProtFrag 在 Zenodo `13373108` | `CODE_VERIFIED` |
| **Proteo-AA** | `NEEDS_ADAPTER` —— 任务形态（受体条件 + 出肽的序列和全原子结构）与我们兼容，**而且它是这一类里唯一直接考全原子 RMSD 的**；但要过滤掉含非标准 AA 的案例，否则我们跑不了那些题 | — |

### DiffPepBuilder

| 字段 | 内容 | 标记 |
|---|---|---|
| **test cases** | **仓库里和论文里是两套不同的东西**：<br>① 论文的 **PepPC-HF 30 个重建复合物**（PDB 编号见 §9.10a）<br>② 仓库 `datasets/docking/docking_benchmark.csv`：**174 行**，列是 DockQ/RMSD × {DiffPepDock, ADCP, FlexPepDock, AF2, DiffDockPP, AF3(三种 MSA/模板组合)}，另有 `ss_type` 列 | ① `EXPLICIT_SOURCE`<br>② `CODE_VERIFIED` |
| **注意** | ②**是肽对接基准不是设计基准**——给定肽找位姿。别当设计成绩引用 | `CODE_VERIFIED` |
| **evaluator** | 无结构预测验证器；L-RMSD、序列相似度、**Rosetta ddG**、**有效率 = ddG<0 的比例** | `EXPLICIT_SOURCE` |
| **Proteo-AA** | `NEEDS_ADAPTER`（重建任务）；对接那套 `OUT_OF_SCOPE` | — |

## 14.4 类别 B：cyclic / macrocyclic

| | RFpeptides | Latent-X 大环肽档 |
|---|---|---|
| **靶点** | 4 个：MCL1、MDM2、GABARAP、**RbtA**（仅预测结构） | 3 个：MDM2 `4hfz`、MCL-1 `2pqk`、PD-L1 `5o45` |
| **长度** | — | **12–18 aa** |
| **验证器** | **AfCycDesign**（环状位置编码的 AF2）；MCL1 另用环状编码的 RF2 | **Chai-1** |
| **阈值** | 逐靶点重调：通用 iPAE<0.3（归一化，≈9.3 Å）+ Cα RMSD<1.5；GABARAP 档收紧到 iPAE<0.13、ddG<−30、SAP<35、CMS>300 | **整个丢掉 `ptm_binder`**（已验证大环肽的 ptm_binder 只有 0.18–0.22），只看 `complex_rmsd` 和 `min_ipae` |
| **协议在哪** | RFdiffusion 仓库 `examples/design_macrocyclic_binder.sh`，开关 `inference.cyclic` + `inference.cyc_chains` | 无（商业闭源） |
| **标记** | `EXPLICIT_SOURCE` + `CODE_VERIFIED`（脚本） | `EXPLICIT_SOURCE` |

**两处都印证了同一件事**：大环肽需要专门的环状位置编码，且**常规置信度指标在大环肽上失效**（Latent-X 直接弃用 ptm_binder）。

**Proteo-AA：`FUTURE`。** 见 §14.1——不是适配问题，是位置编码层的模型改动。

> PXDesign 技术报告提到 12 个环肽靶点，但**我们树里的 PXDesign 代码没有任何环肽支持**（全仓库搜 `cyclic` 只命中一条无关的文献标题）。该 12 靶点集合 `UNRESOLVED`。

## 14.5 类别 D：unconditional peptide generation

**在目前核实的工作中，尚未找到被广泛复用的 benchmark 或约定俗成协议。** `UNRESOLVED`

对比：单体那边至少有一套被反复引用的协议（长度 {100…300} + 每骨架 8 条 ProteinMPNN 序列 + ESMFold + 最小 scRMSD），肽这边我们没有找到对应物。

可能的原因（`INFERRED`）：肽的评价往往绑定某个靶点，脱离靶点的短肽缺少可判定的成功标准。但**这只是一个未找到的结论，不等于不存在**——这一类我们检索的深度低于其他几类。

## 14.6 固定 test set vs 论文自带协议（问题一的回答）

| 名称 | 是固定 test set 吗 | 清单在哪 | 有独立维护/排行榜吗 |
|---|---|---|---|
| **PepGLAD 的 PepBDB 划分** | **是**，190 条随仓库 | `datasets/pepbdb/test.txt` `CODE_VERIFIED` | 否 |
| **PepGLAD 的 LNR 93 条** | **是**，但要从 Zenodo 取 | Zenodo `13373108` `EXPLICIT_SOURCE` | 否；LNR 本身出自 Tsaban 2021，被多家复用 |
| **BOND-PEP 193 对** | **是**，但只在 Zenodo，无 GitHub | Zenodo `19841318` `UNRESOLVED`（未下载） | 否 |
| **DiffPepBuilder PepPC-HF 30** | **是**，PDB 编号在论文 | 论文 Table S1 `EXPLICIT_SOURCE` | 否 |
| **DiffPepBuilder 对接基准 174** | **是**，随仓库 | `datasets/docking/docking_benchmark.csv` `CODE_VERIFIED` | 否；且是对接不是设计 |
| **RFpeptides 4 靶点** | **不是固定 test set**，是论文自己的实验面板 | 论文 | 否 |
| **Latent-X 大环肽 3 靶点** | 同上 | 论文 | 否 |

**结论：这一类里没有任何一个达到 MotifBench 那种"固定题面 + 独立维护 + 排行榜"的水平。** 最接近的是 PepGLAD 的两套划分（清单公开、被同系列论文复用），但没有维护方也没有榜。

## 14.7 重建还是 de novo（问题二的回答）

| 类别 | 性质 |
|---|---|
| PepGLAD（两套划分）、DiffPepBuilder PepPC-HF | **天然复合物重建**——有真值肽，评分方式是"你重建得像不像原来那条" |
| BOND-PEP | **介于两者之间**：案例是天然复合物挖的，但判据是"**超过**天然肽自己的 ipTM"，所以是 de novo 生成 + 以天然为基线 |
| RFpeptides、Latent-X 大环肽 | **de novo target-conditioned**，无真值可比 |

**这个区分很实际**：重建类基准对共设计模型是友好的（可以直接比全原子 RMSD 和序列恢复率），但它测的是"能不能复现已知答案"，不是"能不能设计新东西"。

## 14.8 evaluator 与阈值类型（问题四、五的回答）

| 基准 | evaluator | 阈值类型 |
|---|---|---|
| PepGLAD | **无结构预测器**，直接对晶体结构 | **绝对**：RMSD ≤2/5/10 Å 分档；ΔG < 0 |
| DiffPepBuilder | **无结构预测器**，Rosetta | **绝对**：ddG < 0 |
| BOND-PEP | AF-Multimer / ColabFold | **相对**：超过该靶点天然肽自己的 ipTM，**逐靶点浮动** |
| RFpeptides | AfCycDesign（环状 AF2）+ Rosetta | **绝对但逐靶点重调**（GABARAP 档比通用档严一倍以上） |
| Latent-X 大环肽 | Chai-1 | **绝对**，且**对大环肽弃用了 ptm_binder 这一项** |

**三种阈值哲学同时存在**：绝对固定、逐靶点相对、逐靶点重调。报数时必须说明是哪种，否则"成功率"无法解释。

## 14.9 与 binder 线的信息重叠度（annotation，非取舍结论）

> **这一节是标注，不是这一轮的主结论。** 记录的是"相对 §9 有多少新增信息"，供日后选型时参考；**不构成"某个 benchmark 不值得记录或不值得跑"的判断**。

| 类别 | 与 §9 的关系 | 能力备注 |
|---|---|---|
| **E miniprotein binder** | 任务设定重合，inventory 已合并（§14.0） | — |
| **A linear peptide binder** | 同为 target-conditioned 设计，尺度更小 | 产物是序列，**不评价结构输出** |
| **B cyclic / macrocyclic** | 不重叠 | 需要环状位置编码，属独立的拓扑能力维度 |
| **C 重建 / 条件生成** | 不重叠——**有真值肽**，可直接量全原子 RMSD 与序列恢复率 | 提供 binder 线结构上给不了的一类证据 |
| **D 无条件肽** | — | 未找到被广泛复用的 benchmark 或协议 |

**观察**：A 类与 §9 在任务设定上重叠度高（都是 target-conditioned binder 设计，只是尺度更小）；C 类提供的是**另一类证据**——因为有真值肽，可以直接量"全原子输出与晶体结构差多少"，这是 binder 线结构上给不了的。B 类考的是新拓扑能力。

这些是重叠度的描述，不是优先级排序。

## 14.10 各条目的能力标注（annotation）

> **这是标注不是取舍。** 记录每个 benchmark 实测什么能力、领域怎么用、与其他是否重复、我们能否公平跑、跑了覆盖我们哪部分、有无理由预期表现好坏。分级标签保留在末尾作备注。

### PepGLAD / PepBench

- **实测能力**：给受体，重建那条肽的序列与全原子结构。**这一类里唯一直接量全原子 RMSD + 序列恢复率的**
- **领域怎么用**：PepFlow、D-Flow、PPFlow 等同系列工作复用它的划分；LNR 本身出自 Tsaban 2021，被多家引用
- **重复度**：与 DiffPepBuilder 的重建集同质；与 §9 binder 线**不重复**（binder 线没有真值肽）
- **我们能否公平跑**：能，但**要先过滤含非标准氨基酸的案例**（索引第四列就是这个标志位）。过滤后是标准 20AA 线性肽，我们的表示支持
- **覆盖我们哪部分**：共设计（序列+结构一起出）、全原子精度、短链条件生成。**不覆盖**：从头设计新功能、长链、拓扑
- **预期表现**：`UNRESOLVED`。肽只有 4–25 残基，远短于我们训练分布的主体（单体+复合物），短链上的表现没有先验

### BOND-PEP

- **实测能力**：**只测序列设计这一支**——给靶点直接出肽序列，**不评价任何结构输出**
- **领域怎么用**：2026 年新工作，尚未见第三方复用（`UNRESOLVED`）
- **重复度**：任务与 §9 binder 线同类（target-conditioned），但产物是序列不是结构
- **我们能否公平跑**：能跑，但**只会用到我们的 AA head，结构分支的输出不进评分**。这不是"不值得测"，而是**它测不到我们的结构能力**——如果目标是单独考察序列设计质量，它是合适的
- **协议本身的价值**：相对阈值（超过天然肽自己的 ipTM）是个值得注意的设计——它把"靶点难易"这个混杂因素消掉了
- **预期表现**：`UNRESOLVED`

### DiffPepBuilder

- **实测能力**：仓库和论文是两套。论文的 PepPC-HF 30 个是**重建**；仓库的 174 行是**肽对接**（给定肽找位姿）
- **领域怎么用**：对接那套带了 ADCP、FlexPepDock、AF2、AF3 三种配置的对照，是个横比资源
- **重复度**：重建那套与 PepGLAD 同质；对接那套是**另一个任务**，本调研范围之外
- **我们能否公平跑**：重建可以（同 PepGLAD）；对接**不适用**——我们不做位姿预测
- **覆盖我们哪部分**：同 PepGLAD，且案例更少（30 vs 93/190）

### RFpeptides / Latent-X 大环肽

- **实测能力**：大环肽的 de novo 靶点条件设计，含**环化拓扑**
- **领域怎么用**：RFpeptides 的协议随 RFdiffusion 仓库发布（`inference.cyclic` + `inference.cyc_chains`），是这一类的参考实现；Latent-X 用 Chai-1 并**对大环肽弃用 ptm_binder**
- **重复度**：与其他几类都不重复，是独立的能力维度
- **我们能否公平跑**：**不能**。我们的相对位置编码没有环状模式，生成出来的环肽首尾不闭合（§14.1）。这是能力缺口不是选型问题
- **值得记录的观察**：常规置信度指标在大环肽上会失效（已验证的大环肽 ptm_binder 只有 0.18–0.22），这对任何想扩到环肽的工作都是有用信息

### miniprotein binder

- 见 §14.0：在目前收集到的 benchmark 中与 §9 任务重合，inventory 合并处理

### 分级备注（provisional，仅供日后选型参考）

`POSSIBLE`：PepGLAD 的重建划分——提供 binder 线给不了的一类证据（对晶体结构的全原子偏差），任务形态兼容。
`SURVEY_ONLY`：BOND-PEP（只覆盖序列分支）、DiffPepBuilder（与 PepGLAD 同质 + 对接跑题）、RFpeptides / Latent-X 大环肽（能力缺口）。

> 这些标签表示**当前选型参考**，不表示这些工作在领域里不重要或不该记录。

---

# 十五、配方明细：enzyme / ligand-conditioned

第四轮核实（2026-09-13）。**landscape survey**——记录领域里怎么做，Proteo-AA 兼容性作为附加列。不实现。

## 15.0 五类任务，不要混为一谈

"酶设计基准"这个说法底下其实是五种不同的任务，条件化对象、产物和判分方式都不同。

| 类别 | 条件是什么 | 产物 | 代表 |
|---|---|---|---|
| **A. catalytic motif scaffolding** | 催化残基的原子级 motif（theozyme）+ 配体 | 包住它的支架 | **AME** |
| **B. ligand-conditioned protein generation** | 一个配体/辅因子/金属 | 能结合它的蛋白 | RFdiffusionAA 四配体、**Studio-179**、BoltzDesign1 |
| **C. enzyme / substrate-conditioned design** | 底物 + EC 号 + 功能位点 | 酶序列（±坐标） | EnzyGen/EnzyBench、EnzyControl/EnzyBind |
| **D. pocket redesign** | 配体 + **蛋白其余部分** | 重设计的口袋残基 | PocketGen、PocketFlow |
| **E. sequence / function generation** | 家族或功能描述 | 序列 | EnzyGen 的序列分支；评价靠 ESP / CLEAN / kcat 预测器 |

**A 和 D 的区别值得强调**：A 是从零搭支架（scaffold 不存在），D 是**给定整个蛋白只改口袋那几个残基**——后者的搜索空间小得多，两者的成功率不可比。

**横切工具**（不是任务）：**PoseBusters** 做配体构象合法性检查；**COMPSS** 做指标对实测活性的校准。

## 15.1 复用情况——哪些集合真的被别人跑了

这是本轮新增的维度。用本地论文全文交叉检索得出。`CODE_VERIFIED`（对已有语料而言）

| 集合 | 提出方 | 在本地语料里出现于 | 判断 |
|---|---|---|---|
| **四配体 SAM/OQO/FAD/IAI** | RFdiffusionAA | RFAA、AtomFlow、DISCO、ODesign | **最强**：四篇、多个独立课题组 |
| **AME 41** | RFdiffusion2 | RFdiffusion2、ODesign、MotifBench（提及）；另据二手 RFdiffusion3（37/41）与 Proteína-Complexa | **强**：跨课题组复用，且是**唯一有固定案例清单的酶 benchmark** |
| **CrossDocked / Binding MOAD 各 100** | PocketGen 系 | PocketGen、PocketFlow（**同一作者**） | **中**：本语料内未见独立课题组复用；但这两个数据集在分子生成文献里广泛使用（本轮未抽样该文献） `UNRESOLVED` |
| **Studio-179** | DISCO | **仅 DISCO 自己** | **无第三方复用**（截至本轮检索） |
| **PoseBusters** | 独立 | RFAA、AtomFlow、DISCO | 作为**检查器**广泛复用 |

> 一个可能有用的观察：**这一类里"被复用"和"有固定清单"不是一回事**。四配体复用最广，但它只是四个分子的**惯例**，没有统一的样本数、验证器或阈值——各家跑法不同，数字不可横比。AME 反过来：清单固定、判据明确，复用面稍窄但可比性强。

## 15.2 类别 A：AME（Atomic Motif Enzyme）

| 字段 | 内容 | 标记 |
|---|---|---|
| **task** | 给催化活性位点的原子级 motif（若干残基的部分原子）+ 配体/辅因子/金属，生成容纳它的酶支架 | `EXPLICIT_SOURCE` |
| **test set** | **41 个活性位点**。`mcsa_41.json` 41 条 + `benchmark/input/mcsa_41/` 41 个 PDB，**随仓库发布** | `CODE_VERIFIED`（本轮直接从 `RosettaCommons/RFdiffusion2` 稀疏检出核对） |
| **case selection** | M-CSA 的 958 个手工整理催化位点 × PARITY 交叉比对，取"所有反应物和辅因子都在晶体结构里"的，覆盖 **EC 1–5 类**（占 M-CSA 的 96%）。各 EC 类的具体分布只在图 3a，正文无数字 `UNRESOLVED` | `EXPLICIT_SOURCE` |
| **conditioning** | 催化残基的**随机连通原子子集**（不是整残基）+ 配体。另支持 `++inference.partially_fixed_ligand`——**只给部分配体原子**，其余由模型补 | `EXPLICIT_SOURCE` |
| **sample protocol** | 每案例 **100 条骨架** | `CODE_VERIFIED`（`sweep.num_per_condition: 100`） |
| **sequence route** | **LigandMPNN 每骨架 8 条序列**（`use_ligand: True`，`omit_AA: XC`），并负责侧链打包 | `CODE_VERIFIED` |
| **evaluator** | **Chai-1**，每条序列 **5 个扩散样本**。作者说明选 Chai-1 而非 AF2 的理由是"Chai-1 的侧链相互作用距离更接近天然分布" | `EXPLICIT_SOURCE` |
| **success criterion** | 至少一条序列的某个 Chai-1 样本满足：按催化残基骨架对齐后**全部重原子 RMSD < 1.5 Å**，且**与配体无撞车（两原子 < 1.5 Å）** | `EXPLICIT_SOURCE` |
| **aggregation** | 通过的案例数 / 41。RFdiffusion2 41/41、RFdiffusion 16/41；RFdiffusion3 报 37/41（其判据复述**省掉了撞车那条**） | `EXPLICIT_SOURCE` + `SOURCE_CODE_CONFLICT` |
| **code / data** | `github.com/RosettaCommons/RFdiffusion2`（MIT）。全量跑 41×100×8 = **32,800 次 Chai 折叠**，README 自陈"单机跑完时间不可接受" | `CODE_VERIFIED` |
| **wet-lab** | **有**：三四个催化位点做到台面，"每种情况测试不到 96 条序列就找到活性催化剂"；水解酶 kcat/KM 报到 53,000 M⁻¹s⁻¹ | `EXPLICIT_SOURCE` |
| **复用** | 强（见 §15.1） | — |
| **Proteo-AA** *(附加列)* | `FUTURE`。需要两样我们现在没有的：**配体条件化**（`token_bonds` 未接、无配体表示）和**残基级原子固定**（§13.4，机制在 stage4 分支）。它测的能力我们目前不具备，不是接口问题 | — |

> **已知的代码-论文不一致** `SOURCE_CODE_CONFLICT`：对齐原子集在代码里是 `['N','CA','C','O']`（含 O），论文写 N/Cα/C；`has_valid_ccd()` 只认 41 里的 30 个，另 11 个的配体位姿指标返回空。所以"41/41"是**催化残基 RMSD 那条判据**的成绩，配体位姿只覆盖 30 个。

## 15.3 类别 B：ligand-conditioned protein generation

### 四配体惯例（SAM / OQO / FAD / IAI）

| 字段 | 内容 | 标记 |
|---|---|---|
| **test set** | **4 个分子，不是 benchmark 而是惯例**。选择理由：FAD 和 SAM 在训练集里常见，**IAI 和 OQO 与训练分布很不同**（Tanimoto ≤ 0.5：IAI 0.50、OQO 0.46）——即一个"见过/没见过"的对照 | `EXPLICIT_SOURCE` |
| **sample protocol** | RFdiffusionAA 每配体 **400 条设计**；RFdiffusion3 复跑时同样 400 条 | `EXPLICIT_SOURCE` |
| **sequence route** | LigandMPNN 每骨架 8 条 | `EXPLICIT_SOURCE` |
| **evaluator / 阈值** | **各家不同**：RFdiffusionAA 用 **AF2 单序列**，判据 `AF2 骨架 RMSD < 2 Å`（八条取最优）；RFdiffusion3 用 **AF3**，判据 `骨架对齐配体 RMSD < 5 Å、骨架 RMSD < 1.5 Å、最小链对 PAE < 1.5、ipTM > 0.8`；BoltzDesign1 用 **AF3**，只有 `i_ptm > 0.5` + `complex_plddt > 0.7` | `EXPLICIT_SOURCE` |
| **可比性** | **低**。同样四个分子，三家的验证器和阈值都不同，数字不能横比 | `EXPLICIT_SOURCE` |
| **wet-lab** | RFdiffusionAA 有三场（地高辛配基 Kd 10 nM、血红素、胆色素） | `EXPLICIT_SOURCE` |
| **复用** | 最广（四篇） | — |
| **Proteo-AA** *(附加列)* | `FUTURE`（配体条件化） | — |

### Studio-179（DISCO）

| 字段 | 内容 | 标记 |
|---|---|---|
| **test set** | **179 = 170 个配体 + 9 个多配体组合**；170 个 `.sdf` 随仓库（`priority_0..3/`）。含辅因子、金属簇、污染物、药物、荧光团、有机金属，以及**一个显式的卡宾过渡态结构** | `CODE_VERIFIED` |
| **conditioning** | 配体坐标**参与共折叠**（DISCO 在生成中调整配体坐标），不是固定 | `EXPLICIT_SOURCE` |
| **sample protocol** | 每配体在 150/200/250 三个长度各 50 条 = **150 条设计** | `EXPLICIT_SOURCE` |
| **sequence route** | **无独立逆折叠**——DISCO 序列结构共设计 | `EXPLICIT_SOURCE` |
| **evaluator / 判据** | **Chai-1**；共设计性 = 骨架 RMSD < 2.0 Å **且每个配体质心 RMSD < 2.0 Å**（多配体需逐个满足） | `EXPLICIT_SOURCE` |
| **PoseBusters 的角色** | **不是过滤器**，是对生成配体构象的**报告性诊断**（图 S18 标题明写"未做共设计过滤的结果"） | `EXPLICIT_SOURCE` |
| **code / data** | `github.com/DISCO-design/DISCO`。⚠️ **任务 JSON 是坏的**：`split_1.json` 格式非法；240 个路径只有 1 个能解析（指向不存在的 `xtb/`、`ideal/` 子目录 + 70 个作者集群绝对路径）。**236/239 可按 basename 找回，约十行脚本可修** | `CODE_VERIFIED` |
| **wet-lab** | **有，且很强**：仅以反应中间体为条件设计血红素酶，环丙烷化 72% 产率 / TTN 4,050，B–H 插入 98% / 5,170 TTN | `EXPLICIT_SOURCE` |
| **复用** | **无第三方复用**（截至本轮） | — |
| **Proteo-AA** *(附加列)* | `FUTURE`（配体条件化）。但它是**配体广度最大**的集合，若将来补上配体能力，这是覆盖面最好的一个 | — |

## 15.4 类别 C：enzyme / substrate-conditioned design

| | EnzyGen / EnzyBench | EnzyControl / EnzyBind |
|---|---|---|
| **test set** | **1,500 条**（30 个三级 EC 类 × 50），覆盖 323 个四级家族 | 11,100 对酶–底物（源自 PDBbind）；**测试划分大小 `UNRESOLVED`**——摘要、§3、§5.1、附录、仓库配置均未写出 |
| **清单在哪** | **不随仓库**，Google Drive | 仓库只有 `metadata/demo.csv` **190 行**；全量在 Zenodo 15462173（1.34 GB） |
| **conditioning** | 小分子底物（坐标+特征）+ 四级 EC 标签 + 功能位点索引 | 底物 + EC 号 + 功能位点二值掩码 |
| **产物** | **序列 + Cα 坐标**（不是全原子） | **骨架**（无侧链） |
| **sequence route** | 模型直接出序列，无逆折叠 | 生成骨架 → **ProteinMPNN** → **ESMFold** |
| **evaluator / 判据** | **ESP score ≥ 0.6**（论文明写"0.6 以上表示底物结合为阳性"）；另报 Gnina 对接亲和力、AF2 pLDDT（**这两个无阈值**） | 设计性 = **scRMSD < 2 Å** 的比例；另报 scTM > 0.5 比例、**CLEAN 预测的 EC 匹配率**、kcat 预测、GNINA 亲和力、ESP |
| **防泄漏** | 50% 序列同一性聚类，验证/测试不与训练同簇 | **CD-HIT** 聚类，训练测试不相交 |
| **wet-lab** | **无**，论文明说这些是"湿实验之前"的评估 | **无**，功能性结论全部来自预测器 |
| **复用** | 本语料内未见第三方复用 `UNRESOLVED` | 同左 |
| **Proteo-AA** *(附加列)* | `FUTURE`（底物条件化）。注意 EnzyGen 只出 Cα，**即使能跑也只覆盖我们骨架那一层** | `FUTURE` |

> **这一类的共同特点**：判分依赖**预测器**（ESP、CLEAN、kcat 模型、对接分数）而不是结构自洽或湿实验。这是一条与前面几类不同的评价路线——它试图直接预测"功能"，代价是引入了预测器自身的误差，而这些预测器的可靠性正是 COMPSS 在质疑的（§15.6）。

## 15.5 类别 D：pocket redesign

| | PocketGen | PocketFlow |
|---|---|---|
| **task** | 给配体 + **蛋白其余部分**，重设计口袋残基。口袋 = 配体任意原子 **3.5 Å** 内的残基（平均约 8 个） | 同 |
| **test set** | CrossDocked 100 + Binding MOAD 100 | 同（引用同一划分）；另加 PPDBench 133 个蛋白–肽、PDBBind RNA 56 对 |
| **清单在哪** | **不随仓库**，靠脚本重生成（`split_pl_dataset.py`，seed 2021）。⚠️ 默认带 `--fixed_split` 时 100 口袋的分支被跳过 | **仓库是空的**（只有 251 字节 README） |
| **sample protocol** | 每复合物 100 条序列+结构，三次独立运行 | 每复合物 100 条，三次独立运行 |
| **evaluator / 判据** | AAR；Vina（kcal/mol）；**成功率 = 结合亲和力优于参考口袋的比例**（相对阈值）；ESMFold pLDDT；scTM（8 条 ProteinMPNN → ESMFold） | AAR；**scRMSD**（8 条取最小，**报原始值不设阈值**）；Vina；蛋白–肽用 Rosetta ΔΔG，蛋白–RNA 用 Rosetta-Vienna RNP-ΔΔG |
| **wet-lab** | 无 | 无 |
| **Proteo-AA** *(附加列)* | `FUTURE`（配体条件化）。另注意**这是重设计不是从头生成**，搜索空间比 A 类小得多 | `FUTURE`；且仓库空，今天不可复现 |

> 报数注意：PocketGen 的预印本表 2、README 表格、PocketFlow 论文里引用的同一批实验，**报的列不同数值也不同**（AAR/RMSD/Vina vs AAR/Designability/Vina vs AAR/scRMSD/Vina）。引用时必须说明是哪张表。`SOURCE_CODE_CONFLICT`

## 15.6 横切：验证工具与指标校准

### PoseBusters（配体构象合法性检查器）

不是设计基准，是**检查器**，被 DISCO、AtomFlow、RFAA 复用。`CODE_VERIFIED`
PB-valid = 通过全部检查：化学有效性（RDKit 可加载/可 sanitize/分子式与键与手性一致）；分子内（键长键角在距离几何上下界的 **0.75–1.25** 倍、内部撞车 > 下界 0.7 倍、芳香环共面 < 0.25 Å、UFF 能量不超过 50 构象系综均值的 100 倍）；分子间（蛋白–配体最小距离 > **0.75 × vdW 半径和**、体积重叠 < **7.5%**）。
**模式很重要**：`redock.yml` 需要真值配体；**设计管线要用的是 `dock.yml`**。`pip install posebusters` 即可，纯 CPU。

### COMPSS（把 in-silico 指标对实测酶活性校准）

不是基准，是**方法学研究**，但它回答的问题对整类工作都成立。`EXPLICIT_SOURCE`

20 个指标对实测活性的 AUC：**Rosetta-relax 0.76、ProteinMPNN 0.75**、MIF-ST 0.72、ESM-IF 0.70、ESM-1v 0.68、AF2 pLDDT 0.66；净电荷 0.39、SASA 0.39（不如瞎猜）。最终选 **ESM-1v + ProteinMPNN**（后者计算最省）。**序列同一性不能预测活性。**
过滤后 74% 有活性，比未过滤对照高 77%（Fisher P=0.00018）。

> 这条与 §6 的主线一致：**酶这边同样存在"in-silico 排名预测不了实验结果"的问题**，而 C 类基准恰恰重度依赖这些预测器。

## 15.7 这一类的整体形态

- **只有 A 类（AME）同时具备固定案例清单、明确判据、跨组复用和湿实验锚定。** 其余要么是惯例（四配体）、要么无第三方复用（Studio-179）、要么清单不随仓库（EnzyGen、EnzyControl、PocketGen）、要么仓库空（PocketFlow）
- **两条评价路线并存**：A/B/D 走"结构自洽 + 几何合法性"，C 走"功能预测器"（ESP/CLEAN/kcat）。后者的可靠性正是 COMPSS 质疑的对象
- **验证器分成两族且不可比**：AME/DISCO/RFD3 用 **Chai-1 或 AF3** 评催化侧链与配体位姿（1.5–2.0 Å）；PocketGen/PocketFlow/EnzyControl 用 **ESMFold** 评骨架自洽（2.0 Å）
- **撞车阈值有四套定义**：AME 1.5 Å、旧 AF2 motif 基准 2.0 Å、DISCO vdW 半径和 − 0.5 Å、PoseBusters 0.75 × vdW 半径和

## 15.8 Proteo-AA 兼容性标注（附加，非取舍结论）

**整类目前都是 `FUTURE`，但原因按任务不同，不该一律归到"缺配体条件化"。** `CODE_VERIFIED`（缺口本身）

| 类别 | 主要缺什么 | 具体依据 |
|---|---|---|
| **A 催化 motif 支架** | **配体条件化 + 残基级原子固定**，两样都缺 | 无配体表示；残基级固定的机制在 `stage4.py`，不在本线（§13.4） |
| **B 配体条件生成** | **配体条件化**（这一类唯一的条件就是配体） | 训练数据无小分子/金属簇；`pxdesign_train` 从不引用 `token_bonds` |
| **C 底物 + EC / 功能条件设计** | **底物表示 + 功能级条件化**。即使配体问题解决，还缺**把 EC 号、功能位点索引作为条件**的接口；而且判分依赖 ESP/CLEAN/kcat 这类外部预测器，我们也没接 | EnzyGen 的条件是"底物坐标+特征 + 四级 EC 标签 + 功能位点索引" |
| **D 口袋重设计** | **配体条件化**；但**不需要**从零搭支架的能力——它给定蛋白其余部分，与我们的"给上下文设计一部分"形态更接近 | 口袋 = 配体 3.5 Å 内约 8 个残基 |
| **E 序列 / 功能生成** | **功能或家族级条件化**，与结构条件化是不同的接口；判分同样走功能预测器 | 这一支基本不依赖配体坐标 |

换句话说：**B 和 D 是纯配体问题，A 还叠了固定原子，C 和 E 的主要障碍其实是功能级条件化和功能预测器接口，不是配体本身。**

**这不影响它们作为 landscape 的记录价值。** 若将来补上配体条件化：
- **AME** 会是首选参照——唯一有固定清单 + 跨组复用 + 湿实验的
- **Studio-179** 配体广度最大（170 个，含过渡态结构），适合考覆盖面，但要先修它的任务 JSON
- **D 类（pocket redesign）**门槛最低——给定蛋白其余部分，只改口袋，比从零搭支架容易得多；但它测的是重设计不是生成

**一处可能对我们有利的观察**：AME 的 `partially_fixed_ligand` 模式（只给部分配体原子、其余由模型补）在形态上接近我们"给部分原子、补全其余"的侧链任务。若将来接配体，这条路径的语义差距可能小于从零做配体生成。`INFERRED`

---

# 十六、配方明细：antibody / nanobody

第五轮核实（2026-09-13）。**landscape survey**，不实现。

## 16.0 六类任务，以及两条最容易被混掉的界线

| 类别 | 条件是什么 | 产物 | framework 状态 |
|---|---|---|---|
| **A. 结构预测 / 对接** | 抗体 + 抗原序列或结构 | 复合物结构 | **不设计**，纯预测 |
| **B. CDR loop 设计（尤其 H3）** | 天然复合物 + 完整 framework | 被挖空的 CDR | **完全给定** |
| **C. 序列设计 / 逆折叠** | 复合物骨架 | 全序列或 CDR 序列 | 骨架给定 |
| **D. 抗体–抗原共设计 / binder 设计** | 抗原 + 表位 | CDR 序列与结构 | **给定或模板化** |
| **E. 亲和力成熟 / 局部重设计** | 已有复合物 | 少量突变 | 给定 |
| **F. nanobody / VHH** | 同 B/D，但**单链无轻链** | 同 | 给定或模板化 |

### 界线一：没有一个基准在做"完整抗体从头设计"

**在本轮核实范围内，framework 从来不是生成出来的。** `CODE_VERIFIED`

三种形态，要分清：

| 形态 | 谁 | framework 来自 |
|---|---|---|
| **改某个具体天然抗体的 CDR** | IgGM、dyMEAN、DiffAb、ProteinBench、CHIMERA-Bench | **那一条天然抗体自己的**，且有真值可比 |
| **在通用 framework 模板上从头设计 CDR** | Germinal、EasyNano | **通用模板**（`nb.pdb` / `scfv.pdb`），无真值 |
| **framework + CDR 全生成** | **未找到** | — |

直接证据：IgGM 的设计 FASTA 里 framework 逐残基完整给出，只有 CDR 被 `X` 挖空（纳米抗体 h_cdr3 档正好 15 个 `X`），抗原链也完整给出。`CODE_VERIFIED`

Germinal 虽自称 de novo，但配置里写死 `fw_lengths: [25,17,38,14]`、`cdr_lengths: [11,8,18]`，并 `rm_template_seq: true`——**是"固定 framework 拓扑 + CDR 从头"**，不是生成 framework。`CODE_VERIFIED`

### 界线二：antibody 与 nanobody 不共用协议

**不是"同一套流程换个输入"，协议结构本身就不同。** `CODE_VERIFIED`

- IgGM 的抗体测试集有 **7 个挖空档**（`all_cdr` + `h_cdr1/2/3` + `l_cdr1/2/3`），纳米抗体只有 **4 个**（`all_cdr` + `h_cdr1/2/3`）——因为纳米抗体没有轻链
- ID 格式里纳米抗体的轻链槽位填 `NA`（如 `8q94_C_NA_A`）
- dyMEAN 的数据预处理**在源头就把纳米抗体丢掉**：`data/download.py:209` 的 `post_process()` 一旦发现轻链或重链为空就提前 `cleanup_none()` 返回，注释原文 `we do not handle nanobodies`
- 一个经验证据：**AntiFold 在 Fab CDR 上 BLOSUM62 相似度 0.703，到 VHH CDR 掉到 0.368**（回到通用 ProteinMPNN 的 0.362 水平）——抗体专用模型迁移不过去

### 界线三：预测/对接 ≠ 生成式设计

下列都是**结构预测或对接**基准，不产生新设计，本节单列但不与 B/D 混算：
abag-benchmark-set（110 个低同源复合物，**明确排除纳米抗体**）、Hitawala & Gray（49 Ab + **60 Nb**，抗体纳米抗体分开两臂）、ABAG-docking（112 核心案例含 14 个单域抗体）、FoldBench 的 Ab–Ag 切片（172 个界面）、dyMEAN 的任务二。

## 16.1 固定 test set / 社区挑战 / 排行榜的标注

这一类里**这三样都存在**，是前面几类都没有的：

| | 有什么 | 标记 |
|---|---|---|
| **RAbD** | **事实标准案例集**，**原始 60 条**。ML 圈常用的 55 = 60 减 `2ghw 3uzq 3h3b 5d96 4etq`（前三个缺链、`5d96` 链 ID 标注错、`4etq` 因 HERN 报错被排除）。清单机器可读两处：`dyMEAN/configs.py` 的 `RAbD_PDB`、`MEAN/summaries/rabd_summary.jsonl` | `CODE_VERIFIED`（本轮直接读 `configs.py` 确认 60 条） |
| **CHIMERA-Bench** | **有随仓库发布的排行榜**：`leaderboard.csv` **135 条数据行**（+表头 136 行），11 个方法 × 3 种划分 × **6 种 CDR**（H1/H2/H3/L1/L2/L3），每格是 `mean±std`。**不是满格笛卡尔积**（满格应 198）——轻链 CDR 只有 15 行/种，说明部分方法只报重链。`sample_data/` 带 12 个复合物、每种划分 train 4 / val 2 / test 6，可直接跑通；全量 2,922 在 HF/Zenodo | `CODE_VERIFIED` |
| **AIntibody** | **CASP 式盲测社区挑战**，29 家机构 511 条 AI 设计抗体，前瞻性、有湿实验（含 KinExA），结果 2026 年发表 | `EXPLICIT_SOURCE` |
| ProteinBench 抗体分支 | HuggingFace 排行榜，结果 CSV 机器可读 | `EXPLICIT_SOURCE` |

> 对比前几类：binder 线只有 ProtDBench 一个带完整数据的复跑；motif 线只有 MotifBench 有榜；**抗体线同时有事实标准案例集、仓库内排行榜、和盲测社区挑战**——这一类的评估基础设施是目前几类里最成熟的。

## 16.2 类别 B：CDR loop 设计

### 共同底座：RAbD

三家（dyMEAN、DiffAb、ProteinBench）都在它上面测，但**各自的过滤不同**，"55"和"60"指的是不同的下游处理。
> ⚠️ 三个条目在 SAbDab 里标注是错的（`2ghw`、`3uzq`、`3h3b`）。**dyMEAN 用手工修过的 PDB 保留全部 60 条，ProteinBench 直接丢掉**。自己写 loader 得到 57 是这个原因。`CODE_VERIFIED`

### dyMEAN 三任务协议

| 字段 | 内容 | 标记 |
|---|---|---|
| **test set** | 任务一 CDR-H3 设计：**RAbD 60**；任务二 结构预测：**IgFold 测试集 70 条**（论文说 51，差异来自 `--filter 111` 与解析失败，**两个数都真实，引用要说明阶段**）；任务三 亲和力优化：**SKEMPI 53 条** | `CODE_VERIFIED`（`configs.py` 三个清单分别 60/70/53） |
| **framework** | 完全给定 | `EXPLICIT_SOURCE` |
| **设计哪些残基** | CDR-H3（也支持多 CDR） | `EXPLICIT_SOURCE` |
| **训练/划分** | SAbDab 3,256 条抗体 / 1,644 簇，MMseqs2 按 **40% CDR-H3 同一性**聚类，9:1，与测试共簇的丢弃 | `EXPLICIT_SOURCE` |
| **evaluator / metric** | **无结构预测验证器**——对着天然结构打分：AAR、CAAR、Cα RMSD、TM-score、lDDT、DockQ | `EXPLICIT_SOURCE` |
| **threshold** | **无**，只排名 | `EXPLICIT_SOURCE` |
| **code** | `github.com/THUNLP-MT/dyMEAN`，三个清单都在 `configs.py` | `CODE_VERIFIED` |
| **第三方复用** | 高——它定义的三任务协议被后续 co-design 论文大量沿用 | `EXPLICIT_SOURCE` |
| **Proteo-AA** *(annotation)* | 需要：**残基级 framework 固定**、**CDR mask**、**H/L 双链处理**。见 §16.7 | — |

### DiffAb

- **19 个复合物**，但**仓库不带清单**——按抗原名在 `sabdab_summary_all.tsv` 上过滤得到（`TEST_ANTIGENS` 五个抗原名）。已在 §10.3 列出复现的 19 条
- **有一个真阈值**：**IMP% = 设计 CDR 的 Rosetta 结合能优于原生 CDR 的比例**，即 ΔΔG < 0 `EXPLICIT_SOURCE`

### IgGM

| 字段 | 内容 | 标记 |
|---|---|---|
| **test set** | **SAb-23H2-Ab 60 条** + **SAb-23H2-Nano 27 条**，Zenodo `13790269` | `CODE_VERIFIED`（本轮直接数行） |
| **conditioning** | **framework 完整给出**（逐残基），只有目标 CDR 被 `X` 挖空；抗原链完整给出 | `CODE_VERIFIED` |
| **挖空档** | 抗体 7 档、纳米抗体 4 档（见 §16.0 界线二） | `CODE_VERIFIED` |
| **case selection** | 训练用 2022-12-31 前全部抗体结构；测试用 **2023-06-30 至 2023-12-30** 发布的，且已剔除与训练相似的序列 | `EXPLICIT_SOURCE` |
| **threshold** | 结构预测的**成功率定义为 `DockQ > 0.23`**；CDR-H3 设计报骨架 RMSD + AAR（36% 恢复率） | `EXPLICIT_SOURCE` |

### ProteinBench 抗体分支

55 条，只做 CDR-H3 重设计，每抗原生成 64 条。**13 个指标**分四组。其 `scRMSD` 值得单独记：用 **IgFold** 折设计序列（**同时输入 H 和 L 链**，以真实非 H3 结构作模板）→ Kabsch 对齐 → **在抗原存在下 Rosetta relax 5 次 × 200 步取最低能量** → 与模型生成的 H3 骨架比 Cα RMSD。**噪声下限已给出：IgFold 对天然结构 relax 后 1.77 Å。** `EXPLICIT_SOURCE`

## 16.3 类别 C：序列设计 / 逆折叠

| 字段 | 内容 | 标记 |
|---|---|---|
| **test set** | **203 个 Fab + 61 个 VHH**，`df_fab_info.csv` / `df_vhh_info.csv` 随仓库，带 CDR 边界 | `CODE_VERIFIED`（本轮数行确认 203 / 61） |
| **case selection** | 时间留出集，**2023-01 至 2024-05** | `EXPLICIT_SOURCE` |
| **framework** | 骨架给定（逆折叠任务本身即如此） | — |
| **最有价值的设计** | 仓库把 Fab 分成**三种条件各一个目录**：带抗原、**把抗原链从模板 PDB 里删掉**、坐标先 relax——用来测"抗原依赖性" | `CODE_VERIFIED` |
| **metric** | 序列恢复率、BLOSUM62 相似度、氨基酸用量偏好、关键残基准确率、**Boltz-1 重折 RMSD**、对 ΔΔG 的 Spearman | `EXPLICIT_SOURCE` |
| **关键结论** | AntiFold Fab CDR **0.703** / VHH CDR **0.368**；LM-Design 0.597 / 0.513；ESM-IF 0.423 / 0.376；ProteinMPNN 0.349 / 0.362 | `EXPLICIT_SOURCE` |
| **threshold** | **无** | — |

## 16.4 类别 D：抗体–抗原共设计 / binder 设计

### CHIMERA-Bench

| 字段 | 内容 | 标记 |
|---|---|---|
| **test set** | **2,922 个复合物 / 2,721 个 PDB**；仓库带 `sample_data/` **12 个复合物**（`.pt` 特征文件，如 `8s6m_H_L_R.pt`，命名即 `PDB_重链_轻链_抗原`），全量在 HF `mansoorbaloch/chimera-bench` 与 Zenodo | `CODE_VERIFIED`（本轮直接数：12 个 `.pt` + 135 条榜行） |
| **划分** | 三种、均簇不相交：`epitope_group` 2338/292/292、`antigen_fold` 2338/292/292、`temporal` 2337/292/293 | `EXPLICIT_SOURCE` |
| **表位聚类规则** | 两个复合物同簇当且仅当**排序后的表位残基标识完全相同**——天然处理不连续表位 | `EXPLICIT_SOURCE` |
| **task** | **表位条件下的 CDR 序列–结构共设计**，两个轨道：CDR-H3（必做）与全 6 CDR | `EXPLICIT_SOURCE` |
| **framework** | 给定 | `EXPLICIT_SOURCE` |
| **metric** | 表头逐列可查：`aar,caar,ppl`（序列）、`rmsd,tm_score`（结构）、`fnat,irmsd,dockq`（界面）、`epitope_f1`（表位）、`n_liabilities`（NG/DG/DS/DD/NS/NT/M 液相风险基序计数）、`chimera_s` 与 `chimera_b`（两个复合分） | `CODE_VERIFIED`（leaderboard 表头逐字段） |
| **两套接触定义** | 标注与 CAAR 用 **4.5 Å 重原子**；Fnat/iRMSD/DockQ/表位 F1 用 **8.0 Å Cα–Cα**（天然与预测对称计算） | `EXPLICIT_SOURCE` |
| **threshold** | **无**，排行榜式 mean±std | `CODE_VERIFIED` |
| **基线** | 11 个方法同设置重训（DiffAb、AbFlowNet、AbMEGD、RADAb、dyAb、MEAN、dyMEAN、RAAD、RefineGNN、AbODE、AbDockGen） | `CODE_VERIFIED` |

### Germinal（de novo CDR，模板化 framework）

| 字段 | 内容 | 标记 |
|---|---|---|
| **靶点** | 4 个：PD-L1、IL-3、IL-20、BHRF1。**仓库只有 PD-L1 和 IL-3（另有 insulin）的靶点文件** | `CODE_VERIFIED` |
| **framework** | **通用模板**：`pdbs/nb.pdb` 与 `pdbs/scfv.pdb`。长度在配置里写死，且**纳米抗体档与 scFv 档是两套不同的数**——VHH `cdr_lengths: [11,8,18]` / `fw_lengths: [25,17,38,14]`；scFv `cdr_lengths: [8,8,13,6,6,9]`（注释写 `heavy, then light`）/ `fw_lengths: [25,17,38,52,17,33,10]`。两档都是 `rm_template_seq: true`、`rm_template_sc: false`——**抹掉模板序列但保留模板侧链**，所以生成的是序列不是拓扑 | `CODE_VERIFIED` |
| **流程** | ColabDesign 幻觉 → **AbMPNN** 选择性重设计 → AF3 / Chai-1 / Protenix 共折叠 | `EXPLICIT_SOURCE` |
| **threshold** | **这一类里唯一有完整阈值级联的**（`configs/filter/final/vhh.yaml`）：`sc_rmsd<6.0`、`interface_shape_comp≥0.6`、`interface_hbonds≥3`、`surface_hydrophobicity≤0.4`、`pdockq2>0.23`、`external_plddt>0.87`、`external_iptm>0.74`、`external_ptm>0.84`、`external_pae<7.5`。scFv 档另有一套 | `CODE_VERIFIED` |
| **wet-lab** | **有**：BLI 确认成功率 **4–22%**，最好 K_D 140–560 nM。**scFv 全部无显著结合信号**——纳米抗体才是被验证的模式 | `EXPLICIT_SOURCE` |
| **重要警告** | 原文明写**阈值只针对 AF3 校准过**，Chai-1 与 Protenix 后端是实验性、未校准 | `EXPLICIT_SOURCE` |
| **第三方复用** | 尚未见 `UNRESOLVED` |

## 16.5 类别 E：亲和力成熟 / 局部重设计

- **SKEMPI 抗体子集 53 条**（dyMEAN 任务三）：给已有复合物做少量突变，指标 ΔΔG（kcal/mol）+ ΔL（改动残基数），每条抗体生成 100 个候选取 top-1。**注意 53 这个数来自 dyMEAN 仓库，不是 SKEMPI 本身** `CODE_VERIFIED`
- **AbBiBench**：**不打恢复率**，指标是**模型对完整抗原–抗体复合物的对数似然与实验亲和力的 Spearman 相关**。当前前三：ProteinMPNN 0.30、ESM-IF1 0.28、AntiFold 0.21——**逆折叠模型赢，纯序列语言模型接近零甚至为负**。**不含任何纳米抗体**（17 个条目全是配对 H/L） `EXPLICIT_SOURCE`
- **FLAb**：241 个数据集、7 类治疗性质、>300 万条实测。纳米抗体子集：AVIDa-hIL6 VHH 573,892 条、SARS-CoV-2 VHH 77,004 条、NbThermo 673 条

## 16.6 类别 F：nanobody / VHH

| 来源 | 规模 | 任务 | 可机读 | 标记 |
|---|---:|---|---|---|
| **IgGM SAb-23H2-Nano** | **27** | CDR 重设计（framework 给定） | ✅ Zenodo，含结构 + 预挖空 FASTA | `CODE_VERIFIED` |
| **逆折叠基准的 VHH 分支** | **61** | 逆折叠 | ✅ CSV 随仓库 | `CODE_VERIFIED` |
| **nanoFOLD** | 43 晶体 + 1064 模型 | 逆折叠恢复率 | ❌ 无清单无仓库 | `EXPLICIT_SOURCE` |
| **Germinal** | 4 靶点 | **de novo CDR**（模板 framework） | ⚠️ 仓库只有 2 个 | `CODE_VERIFIED` |
| **EasyNano** | 5 + 1 | de novo CDR | ❌ 代码未发布（可用性声明里组织名占位符没填） | `EXPLICIT_SOURCE` |
| **NbBench** | 8 任务 | **性质预测为主**（7/8 是预测，只 CDR 填空沾边生成） | ✅ HF | `EXPLICIT_SOURCE` |
| **Hitawala & Gray** | 60 Nb–Ag | **对接**（非设计） | ❌ | `EXPLICIT_SOURCE` |

**观察**：纳米抗体这边**没有出现抗体那边那样的"事实标准案例集 + 排行榜"组合**。IgGM 的 27 条是最大的带结构公开清单，但那是单篇论文的时间留出集。`abag-benchmark-set` 还**明确把纳米抗体排除在外**。

## 16.7 Proteo-AA 兼容性标注（annotation）

**这一类目前整体 `FUTURE`，但缺口与 enzyme 那边完全不同——不是配体，是抗体特有的条件化接口。**

若将来要跑，按任务需要这些能力：

| 能力 | 谁需要 | 我们现状 |
|---|---|---|
| **残基级 framework 固定**（给定 framework 的每个残基坐标与类型，只生成 CDR） | B、D、F 全部 | 机制在 `stage4.py`（原子级掩码 + 每步还原），**不在本线**；且 `token_is_design` 现由**链级** binder 掩码派生，要改成按残基填（§13.4） |
| **CDR mask / 区域标注** | B、D、F | **完全没有**。需要 IMGT/Chothia 编号与 CDR 边界解析（`abnumber`/ANARCI 一类） |
| **H/L 双链抗体处理** | B、D 的抗体轨道 | 多链本身支持（binder 设计就是双链），但**没有"重链/轻链"这一语义**，也没有配对约束 |
| **VHH 单链** | F | 结构上最接近我们现有能力——**单链 + 抗原上下文**，与 binder 设计形态一致 |
| **抗体特异的序列先验** | 全部 | 无。AbMPNN、AntiFold、AntiBERTy 这类抗体专用模型我们没有对应物 |

**一处值得记的形态观察**：**F 类（VHH）在接口上离我们最近**——单链、给定 framework、抗原作为上下文，与我们已经在做的 binder 设计只差"framework 残基级固定 + CDR mask"两件事，不需要新的分子类型支持。相比之下 B/D 的抗体轨道还多一层 H/L 配对语义。`INFERRED`

**另一处**：这一类的评估**大多不需要结构预测验证器**（dyMEAN、DiffAb、CHIMERA-Bench 都是直接对天然结构打分），这对我们是有利的——不用额外搭 AF2/AF3 环境。但代价是它们测的是"**能不能复现已知抗体**"，不是"能不能设计新抗体"。

## 16.8 本轮新增 unresolved

| 项 | 状态 |
|---|---|
| RAbD 从 60 减到 55 的完整筛选规则 | 已知被排除的五条及各自原因，但**规则本身**未见原始定义 |
| CHIMERA-Bench 全量 2,922 的实际内容 | 仓库只带 12 条样本，全量未下载 `UNRESOLVED` |
| Germinal 的 IL-20 与 BHRF1 靶点文件 | 不在仓库，需从预印本重建 |
| nanoFOLD 的 43 / 1064 具体是哪些 | 无清单无仓库 `UNRESOLVED` |
| AIntibody 的题面是否可复用 | 是竞赛不是可下载测试集，`UNRESOLVED` |
| EasyNano 代码 | 未发布 |
